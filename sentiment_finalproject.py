...
rating_data = pd.read_csv("tweets_weightlifting_clean.csv", encoding = 'latin1')
rating_data = rating_data.rename(columns={rating_data.columns[0]: "Sentences" })
sentiment_data = get_sentiment(rating_data)
sentiment_data = sentiment_data[sentiment_data['sent_neu'] != 1]
sentiment_data.to_excel("sentiment_weightlifting.xlsx", index = False)
print (" Written to sentiment_boxing.xlsx")
